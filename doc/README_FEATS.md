...
a multiple feature tagger `mordl.FeatsTagger` that allows to predict all the
content of key-value type fields in a single pass.

For the single feature tagger, refer to the
...
based joint multiple feature prediction model. The latter may have slightly
better performance (but to achieve it, you have to tune other params too),
though on very long sentences it may cause *CUDA out of memory* error on the
inference.
...
**rnn_emb_dim** (`int`; default is `300`): the internal character RNN
